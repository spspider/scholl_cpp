...
public class Breakout1 extends WindowProgram {
    ...
    public void run() {
        ...
        addMouseListeners();
        createFirstStep();
        GObject click = pleaseClickForStart();
        waitForClick();
        remove(click);
        createSecondStep();
        drawBlocks();
    ...
    private GLabel pleaseClickForStart() {
        GLabel clickForStart = new GLabel("PLEASE CLICK TO START",0,0);
        clickForStart.setLocation(getWidth()/2.0-clickForStart.getWidth()/2.0,getHeight()/2.0);
        add(clickForStart);
        return clickForStart;
    }
    ...
    private void endTheGame() {
        GLabel endGame = new GLabel("GAME OVER",getWidth()/2.0,getHeight()/2.0);
        endGame.setFont("serif 25");
        endGame.setLocation(getWidth()/2.0-endGame.getWidth()/2.0,getHeight()/2.0);
        add(endGame);
    }
    ...
    private double addTopBottomWalls(double vy) {
        if ((ball.getY() < 0) || (ball.getY() > getHeight() - BALL_DIAMETER)) {
            vy *= -1;
        }
        if (ball.getY() < 0) {
