...
public class Assignment5Part4 extends TextProgram {
    ...
    private static final String CLOSEBRAKETS = "\",";
    ...
    private String getTextInThatIndex(int number, String s) {
        ...
        for (int i = 0; i < number + 1; i++) {
            ...
            if ((s.charAt(0) == BRACKETS)) {//found brackets
                //we need find close brakets, and close brakets will be ""\+ ,"
                ...
                int closeBrakets = s.indexOf(CLOSEBRAKETS);
                if (nextDecimal > nextBrakets) {//double brackets
                    nextBrakets = nextDecimal-1;
                }
                if (closeBrakets > nextBrakets) {//double brackets
                    addBr+=1;
                    nextBrakets = closeBrakets;
                }
                if ((s.indexOf(BRACKETS) == -1)||(closeBrakets==-1)) {
                    return null;
                }
                found = s.substring(0, nextBrakets);
                    s = s.substring(nextBrakets+addBr);
